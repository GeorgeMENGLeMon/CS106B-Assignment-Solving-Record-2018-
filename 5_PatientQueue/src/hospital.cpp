...
// For this problem, most good extra feature ideas involve adding operations to your queue beyond those specified. Here are some ideas and extra features that you could add to your existing program:
...
// "for example, if you are merging two linked lists, directly connect the node pointers of one to the other as appropriate."

// Doubly-Linked List: Write an implementation using a linked list whose nodes have both prev and next pointers. This makes it easy to move both forward and backward in the list, which can speed up certain operations like changing priority. Also declare both a front and a back pointer that point to the first and last element in the list respectively.
// front back
